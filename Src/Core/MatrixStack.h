...
namespace t3d
{
	...
	public:
		...
		void rotate(Vec3f axis, Float angleDeg) { mCurrentMatrix = glm::rotate(mCurrentMatrix, angleDeg, axis); }
		...
		void scale(Vec3f scaleVec) { mCurrentMatrix = glm::scale(scaleVec); }
		...
		void translate(Vec3f deltaVec) { mCurrentMatrix = glm::translate(deltaVec); }
		void translate(Float dx, Float dy, Float dz) { translate(Vec3f(dx, dy, dz)); }

		//helpers
		void lookAt(Vec3f cameraPos, Vec3f lookatPos, Vec3f upDir) { mCurrentMatrix *= glm::lookAt(cameraPos, lookatPos, upDir); }
		...
		void pixelPerfectOrtho(glm::ivec2 size, Vec2f depthRange, Bool isTopLeft=true);
		
		void applyMatrix(Mat4 matrix) { mCurrentMatrix *= matrix; }
		void setMatrix(Mat4 matrix) { mCurrentMatrix = matrix; }
